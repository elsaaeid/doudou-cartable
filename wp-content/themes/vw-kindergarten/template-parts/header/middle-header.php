<?php
/**
 * The template part for Middle Header
 *
 * @package VW Kindergarten
 * @subpackage vw-kindergarten
 * @since vw-kindergarten 1.0
 */
?>

<div class="container">
  <div class="main-header">
    <div class="row">
      <div class="col-lg-4 col-md-4 align-self-center social-icons text-md-start text-center">
        <?php dynamic_sidebar('header-social'); ?>
      </div>
      <div class="col-lg-4 col-md-4 align-self-center">
        <div class="logo text-center">
          <?php if ( has_custom_logo() ) : ?>
            <div class="site-logo"><?php the_custom_logo(); ?></div>
          <?php endif; ?>
          <?php $blog_info = get_bloginfo( 'name' ); ?>
            <?php if ( ! empty( $blog_info ) ) : ?>
              <?php if ( is_front_page() && is_home() ) : ?>
                <?php if( get_theme_mod('vw_kindergarten_logo_title_hide_show',true) == 1){ ?>
                  <p class="site-title mb-0"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></p>
                <?php } ?>
              <?php else : ?>
                <?php if( get_theme_mod('vw_kindergarten_logo_title_hide_show',true) == 1){ ?>
                  <p class="site-title mb-0"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></p>
                <?php } ?>
              <?php endif; ?>
            <?php endif; ?>
            <?php
              $description = get_bloginfo( 'description', 'display' );
              if ( $description || is_customize_preview() ) :
            ?>
            <?php if( get_theme_mod('vw_kindergarten_tagline_hide_show',false) == 1){ ?>
              <p class="site-description mb-0">
                <?php echo esc_html($description); ?>
              </p>
            <?php } ?>
          <?php endif; ?>
          <svg class="logo-shape" width="317" height="58" viewBox="0 0 317 58">
            <image id="logo-curve" width="317" height="58" xlink:href="data:img/png;base64,"/>
          </svg>
        </div>
      </div>
      <div class="col-lg-4 col-md-4 align-self-center text-md-end text-center pb-md-0 pb-3">
        <?php if(class_exists('woocommerce')){ ?>
          <span class="cart_no">
            <a href="<?php if(function_exists('wc_get_cart_url')){ echo esc_url(wc_get_cart_url()); } ?>" title="<?php esc_attr_e( 'shopping cart','vw-kindergarten' ); ?>"><i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_cart_icon','fas fa-shopping-basket')); ?>"></i><span class="screen-reader-text"><?php esc_html_e( 'shopping cart','vw-kindergarten' );?></span></a>
          </span>
        <?php } ?>
        <span class="search-box">
          <a href="#"><i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_search_icon','fas fa-search')); ?>"></i></a>
        </span>
      </div>
    </div>
    <div class="serach_outer">
      <div class="closepop"><a href="#maincontent"><i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_search_close_icon','fa fa-window-close')); ?>"></i></a></div>
      <div class="serach_inner">
        <?php get_search_form(); ?>
      </div>
    </div>
  </div>
</div>